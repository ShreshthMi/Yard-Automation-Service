 ...
 * Length | Bit | Yard Name | Occupied Message 1 | Occupied Message 2
 *
 * Yard Name / Occupied Message 1 / Occupied Message 2 are only meaningful on
 * identifier rows (e.g. the "1T" row) - they are how a track section is
 * assigned to a yard and given its occupied message/warning directly in the
 * spreadsheet. Leave them blank on any row that isn't part of a yard.
 *
...
public class ConfigLoader {
	...
	public static List<Row> loadCoreDefinition(String xlsxPath) throws IOException {
		...
		for (org.apache.poi.ss.usermodel.Row excelRow : sheet) {
			...
			rows.add(new Row(parent, field, null, type, offset, length, null, null, null, null, excelRowNumber));
		}
		return rows;
	}
	...
	public static List<Row> loadPayloadDefinition(String xlsxPath) throws IOException {
		...
		for (org.apache.poi.ss.usermodel.Row excelRow : sheet) {
			...
			String yardName = text(excelRow, 7);
			String occupiedMessage1 = text(excelRow, 8);
			String occupiedMessage2 = text(excelRow, 9);
			rows.add(new Row(parent, field, identifier, type, offset, length, bit, yardName, occupiedMessage1,
					occupiedMessage2, excelRowNumber));
		}
		return rows;
	}
	 ...
	 * @throws IOException 
	 */
	private static Sheet cloneSheetInMemory(Sheet source, FormulaEvaluator evaluator) throws IOException {
