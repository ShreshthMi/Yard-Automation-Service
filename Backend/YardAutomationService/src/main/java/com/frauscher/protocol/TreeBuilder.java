 ...
 *
 * The sheet is written as a pre-order walk of the tree: every row's
 * "Parent" column names the Field of the row it belongs under, and a
 * group's children are exactly the rows that immediately follow it whose
 * Parent matches the group's own Field name. A single pass with a shared
 * read position builds the whole tree.
 */
